...
def parse_args():
    ...
    parser.add_argument("--do_predict", default=True, help="To do prediction on the question answering model")
    parser.add_argument(
        "--validation_file", type=str, default='valid.json', help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
        "--test_file", type=str, default='new_test.json', help="A csv or a json file containing the Prediction data."
    )
    parser.add_argument(
        "--output_file", type=str, default='prediction.csv', help="A csv or a json file containing the Prediction data."
    )
    parser.add_argument(
...
def main():
    ...
    def prepare_validation_features(examples):
        ...
        examples[context_column_name] = [context_data[r] for r in examples[context_column_name]]
    ...
    if args.do_predict:
        ...
        prediction = post_processing_function(predict_examples, predict_dataset, outputs_numpy, stage='pred')
        
        df_predictions = pd.DataFrame(list(prediction.items()), columns=["id", "answer"])
        df_predictions.to_csv(args.output_file, index=False, encoding="utf-8")

        print("Save QA prediction to output file")
