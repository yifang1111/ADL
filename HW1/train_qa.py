...
def main():
    ...
    def plot_learning_curves(train_losses, val_em, output_dir):
        print(train_losses)
        print(val_em)
        train_losses = [loss.cpu().numpy() for loss in train_losses]
        # val_em = [em.cpu().numpy() for em in val_em]
        # Create figure for loss and Exact Match (EM) plots
        ...
        # Plot training and validation Exact Match (EM)
        ax2.plot(range(1, len(val_em) + 1), val_em, label="Valid")
        ax2.set_title("Learning Curve - Exact Match (EM)")
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        val_em.append(eval_metric["exact_match"])
    
    plot_learning_curves(train_loss, val_em, args.output_dir)
